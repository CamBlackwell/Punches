...
class UnifiedAudioAnalyser: ObservableObject {
  ...
  // DEBUG metrics
  #if DEBUG
  @Published var tapCallbackMaxUs: Double = 0
  @Published var tapOverrunCount: Int = 0
  private let tapOverrunThresholdUs: Double = 1000 // 1 ms budget for tap
  #endif
  ...
  private func installTapSafely(on audioEngine: AVAudioEngine) {
    ...
    mixer.installTap(
      ...
      DispatchQueue.main.async { [weak self] in
        guard let self else { return }
        if us > self.tapCallbackMaxUs { self.tapCallbackMaxUs = us }
        if us > self.tapOverrunThresholdUs { self.tapOverrunCount &+= 1 }
      }
      #endif
    }
  }
